...
As with the Gentzkow and Shapiro model, we assume that there are two states of the world
...
For the remainder of this paper we constrain ourselves to the case where \[\sigma_{ir}(\hat{r}) > \sigma_{il}(\hat{l}).\]
...
Additionally, you have the reports for each of the $N$ firms as evidence.  Then what you might do is update your beliefs following Bayes rule
 ...
 Unfortunately, this does not work in the case considered above because each firm can observe the reports of their fellow firms.  Fortunately, since you have a measure of how much likely it is that each firm will incorporate the reports of another firm into their report, $p_j(\text{high}_i|\hat{\textbf{s}}_{it})$.  We could modify the above posterior to account for this, let $\gamma_ij$ represent $p_j(\text{high}_i|\hat{\textbf{s}}_{it})$
 
 \[p(R|\hat{\textbf{s}}) \propto (1-\gamma_{12})(1-\gamma_{13})...(1-\gamma_{1N})p(\hat{s}_1|R)...(1-\gamma_{N1})(1-\gamma_{N2})...(1-\gamma_{NN-1})p(\hat{s}_N|R)p(R)\]
...
Using this measure we could then think like the Bayesian agent from the section above
...
We consider what proportion of the population of firms are border cases and how much information passes through the border cases.  This sub section may belong in a different section.

\subsection{Risk Averse News Firms}

Next we consider the possibility that there are risk averse media firms.  These are firms that are more likely to trust firms that have a higher quality level controlling for bias.  To estimate who the risk averse firms are we could assume the following relationship between trust, bias and bias-free quality

\[\gamma_{ij} = \beta_1 \theta_{ij} + \beta_2 p(\text{high}_j|\hat{\textbf{s}}_j)\]

Firms that are more risk averse should have a higher $\beta_2$ relative to the rest of the population of firms.  This may also fit better in a different section.
