...
\[p(\theta, \phi, \gamma, \alpha, \beta, z | w) \propto \prod_{i=1}^N \prod_{t=1}^T \prod_{j=1}^{J_{ti}} \prod_{k=1}^K p(w_{itj} | \phi, z_{itj}) p(z_{itj} | \theta_{it}) p(\phi_k | \beta) p(\theta_{it} | \alpha_i, \gamma_i, \delta^2) p(\alpha_i | \chi, \sigma^2) p(\gamma_i | \eta, xi^2)\]
