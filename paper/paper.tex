...
\[= \frac{\theta_{it}^{I_{z_{jit}}=k}}{\sum_k \theta_{it}} \frac{\phi_{t}^{I_{w_{jit}}=v}}{\sum_v \phi_{kt}}\]
...
\subsection{Posterior Specification for $b^2$}

\[p(b^2|\beta,\phi) \propto \prod_{t=1}^T \prod_{k=1}^K p(\phi_{kt} | \beta_t, b^2)p(b^2 | \nu_0, \tau_0) \]

\[= \prod_{t=1}^T \prod_{k=1}^K N(\phi_{kt};\beta_t, b^2)IW(\nu_0, \tau_0)\]

\[\propto IW\left(\nu_0 + TK, \left(\tau_0 + \sum_{t=1}^T \sum_{k=1}^K (\phi_{kt} - \beta_t)(\phi_{kt} - \beta_t)^T\right)^{-1}\right)\]
...
\subsection{Posterior Specification for $a^2$}

\[p(a^2|\alpha,\theta) \propto \prod_{t=1}^T \prod_{i=1}^N \prod_{d=1}^{D_i} p(\theta_{dit} | \alpha_{it}, a^2)p(a^2 | \nu_1, \tau_1) \]

\[= \prod_{t=1}^T \prod_{i=1}^N \prod_{k=1}^K N(\theta_{dit};\alpha_{it}, a^2)IW(\nu_1, \tau_1)\]

This needs to be fixed

\[\propto IW\left(\nu_1 + TND, \left(\tau_0 + \sum_{t=1}^T \sum_{i=1}^N \sum_{d=1}^{D_i} (\theta_{dit} - \alpha_{it})(\theta_{dit} - \alpha_{dit})^T\right)^{-1}\right)\]
...
\[= \frac{1}{\sqrt{2 \pi \delta^2}} \exp\left(-\frac{1}{2\delta^2} \sum_{t=2}^T \left(\alpha_{it} - \alpha_{t-1}^T \gamma_i\right)^2\right)\]
...
\[\propto \exp\left(-\frac{1}{2 \delta^2} \sum_{t=2}^T \left(\alpha_{it} - \alpha_{t-1}^T \gamma_i\right)^2\right) \exp\left(-\frac{1}{2\xi^2}\left(\gamma_i - \eta)^2\right)\right)\]

\[= \exp\left(-\frac{1}{2 \delta^2} \sum_{t=2}^T \left(\alpha_{it}^2 + \left(\alpha_{t-1}^T \gamma_i\right)^2 - 2 \alpha_{it} \alpha_{t-1}^T \gamma_i \right)\right) \exp\left(-\frac{1}{2\xi^2}\left(\gamma_i^2 + \eta^2 - 2 \gamma_i \eta)\right)\right)\]

\[= \exp\left(- \frac{\gamma_i^2}{2}\left(\frac{1}{\xi^2} + \frac{\left(\sum_{t=2}^T \alpha_{t-1}^T\right)^T\left(\sum_{t=2}^T \alpha_{t-1}^T\right)}{\delta^2}\right)\right)\]

\[\exp\left(\gamma_i \left(\frac{\eta}{\xi^2} + \frac{\left(\sum_{t=1}^T \alpha_{t-1}^T\right)^T \left(\sum_{t=2}^T \alpha_{it}\right)}{\delta^2}\right) - \left(\frac{\eta^T \eta}{2\xi^2} + \frac{\left(\sum_{t=2}^T \alpha_{it}\right)^T \left(\sum_{t=2}^T \alpha_{it}\right)}{2 \delta^2}\right)\right)\]

\[\propto N(\mu_{\gamma i}, \Lambda_{\gamma i})\]

\[\Lambda_{\gamma i} = \left(\frac{1}{\xi^2}+ \frac{\left(\sum_{t=2}^T \alpha_{t-1}^T\right)^T\left(\sum_{t=2}^T \alpha_{t-1}^T\right)}{\delta^2}\right)^{-1}\]
...
Since this doesn't have a standard form we use a slice sampler to sample from the full conditional.

\subsection{Posterior Specification for $\phi_t$}

\[p(\phi_t | \beta_t, z, w, b^2) \propto p(w | \phi_t, z) p(\phi_t | \beta_t)\]

\[= \prod_{k=1}^K \prod_{j=1}^{J_k} Mult(w_{jkt}; \pi(\phi_{kt}))\prod_{k=1}^K N(\beta_t, b^2)\]
